...
python -m components.parser.cli parse ./contract.pdf

# Force a strategy
python -m components.parser.cli parse ./scan.pdf --strategy ocr_cloud
python -m components.parser.cli parse ./notes.txt --strategy simple

# Batch parse a directory
python -m components.parser.cli parse-dir ./corpus/ --recursive --out ./parsed/

# List available strategies and provider status
python -m components.parser.cli list-strategies

# Dry-run inspection (no parsing)
python -m components.parser.cli inspect ./scan.pdf

# Debug report for a previously parsed document
python -m components.parser.cli debug <document_id>

# Output formats: json, markdown, table
python -m components.parser.cli parse ./data.csv --output json
```
...
python -m components.parser.api
...
View via: `python -m components.parser.cli debug <document_id>` or `GET /parse/<document_id>/debug`.

## How to Add a New Parser Provider

1. Create a new component in `components/parser/components/` with `@component` and a `run()` method.
2. Register it in `ParserRouter._create_strategy()` in `components/parser/routing/router.py`.
3. Add its config block to `ParserConfig` in `components/parser/config.py`.
